...
import { Music2, Film, User, Mail, Lock, ArrowRight } from 'lucide-react';

const Register = () => {
  const { t, i18n } = useTranslation();
  const navigate = useNavigate();
  const { register, syncGoogleUser } = useAuth();
      ...
      <div className="landing-content animate-fadeIn">
        ...
        <div className="landing-hero">
          ...
          <p className="landing-tagline">
            {t('landing.regHeroTagline')}
          </p>
          <div className="landing-features">
            <div className="feature-pill">{t('landing.regFeat1')}</div>
            <div className="feature-pill">{t('landing.regFeat2')}</div>
            <div className="feature-pill">{t('landing.regFeat3')}</div>
          </div>

          {/* Language Selector in Hero (Aligned with Login) */}
        ...
        <div className="landing-auth-card">
            ...
            <form className="auth-form" onSubmit={handleSubmit}>
              <div className="form-group">
                <div className="input-with-icon">
                  ...
                  <input 
                    type="text" 
                    placeholder={t('landing.regPlaceholderName')} 
                    value={name}
          ...
          <div className="auth-footer">
            <p>{t('landing.regFooterPrompt')} <Link to="/login">{t('landing.regFooterLink')}</Link></p>
          </div>
        </div>
      </div>
    </div>
